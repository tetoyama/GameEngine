#ifndef MATERIAL_LIGHTING_TRAVERSAL_HLSLI
#define MATERIAL_LIGHTING_TRAVERSAL_HLSLI
...
float ShadowFactorPointBiased(
    float3 selectionWorldPos,
    float3 projectedWorldPos,
    int firstLightIdx,
    int faceCount,
    int atlasOffset,
    ShadowPCFParams pcf)
{
    ...
    int selectedFace = SelectPointShadowFace(
        selectionWorldPos - firstFaceLight.Position.xyz);
    ...
    float4 sp = mul(float4(projectedWorldPos, 1.0), faceLight.LightView);
    ...
    ShadowPCFParams shadowParam)
{
    ...
    while (entryIndex < activeEntryCount)
    {
                ...
                light.Dummy == 1)
            {
                shadow = ShadowFactorCascades(
                    ...
                    shadowParam);
                     ...
                     light.Dummy == -1)
            {
                shadow = ShadowFactorPointBiased(
                    input.worldPos,
                    shadowWorldPos,
                    currentEntryIndex,
                    entrySpan,
        ...
        float3 diffuse =
            light.Diffuse.rgb * attenuation * NdotL * toonShadow;
        ...
        float3 specular =
            specBRDF * light.Diffuse.rgb *
            attenuation * NdotL * specularShadow;
...
#endif // MATERIAL_LIGHTING_TRAVERSAL_HLSLI

...
Texture2D g_TextureShadow : register(t1); //法線マップ用
...
void main(in PS_IN In, out float4 outColor : SV_Target)
{
    ...
    lightFactor = saturate((lightFactor + 1.0f) * 0.5f); // 0～1 に正規化

    // 影画像（横方向1D）のUVを生成（例えば横100%を影の段階と見なす）
    float2 shadowUV = float2(1.0f - lightFactor, 0.5f); // y固定（中心ライン）

    float3 shadowSample = g_TextureShadow.Sample(g_Sampler, shadowUV).rgb;

    // 影色として、テクスチャと拡散色を掛けたものを使用
    float3 toonLight = shadowSample * attenuation;

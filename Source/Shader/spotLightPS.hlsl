...
{
    //光源からピクセルへのベクトル
    float3 LightVec = normalize(In.WorldPosition.xyz - Light.Position.xyz);
    ...
    //反射ベクトル
    float3 RefVec = normalize(reflect(normalize(Light.Direction.xyz), normal.xyz));
    ...
    //コーンの向きとLightVecの角度
    float Angle = acos(dot(LightVec.xyz, normalize(Light.Direction.xyz)));

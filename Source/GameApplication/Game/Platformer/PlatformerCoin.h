...
public:
	...
	void OnUpdate(float dt) override {
		...
		t->scale = baseScale;
		t->AddRotationY(rotationSpeed * dt * (4.0f + normalized * 5.0f));
		...
		if(auto* mat = material.TryGet()) {
			...
			mat->Material.BaseColor.z = baseMaterial.BaseColor.z + (1.0f - baseMaterial.BaseColor.z) * flash;
			mat->Material.EmissiveColor = float3(1.0f, 0.65f, 0.08f);
			mat->Material.EmissiveIntensity = baseMaterial.EmissiveIntensity + flash * 4.5f;
		}
...
private:
	void TryCollect(const EntityRef& other) {
		...
		if(auto* t = transform.TryGet()) {
			...
			t->position = basePosition;
			t->scale = baseScale;
			if(auto* effect = particle.TryGet()) effect->particleSize = 0.15f;
		...
		if(auto* cameraController = camera.TryGet()) {
			cameraController->AddImpulse(0.08f, 0.11f, 0.015f, Vector3(0.0f, 1.0f, 0.0f));
		}

...
public:
	...
	CommandQueueType GetQueueType() const noexcept override {
		return m_queueType;
	}
	...
	bool BeginRenderPass(const RenderPassDesc&) override {
		if(!m_recording || m_queueType != CommandQueueType::Graphics){
			return false;
		}
	...
	bool SetPipelineState(PipelineStateHandle pipeline) override {
		return m_recording && static_cast<bool>(pipeline);
	}

	void SetViewport(const Viewport&) override {}

	bool SetVertexBuffer(uint32_t, BufferHandle buffer, uint32_t, uint32_t) override {
		return m_recording && static_cast<bool>(buffer);
	}

	bool SetIndexBuffer(BufferHandle buffer, IndexFormat, uint32_t) override {
		return m_recording && static_cast<bool>(buffer);
	}

	bool SetConstantBuffer(ShaderStage, uint32_t, BufferHandle buffer) override {
		return m_recording && static_cast<bool>(buffer);
	}

	bool SetTexture(ShaderStage, uint32_t, TextureHandle texture) override {
		return m_recording && static_cast<bool>(texture);
	}

	bool UpdateBuffer(BufferHandle buffer, std::span<const std::byte>, uint32_t) override {
		return m_recording && static_cast<bool>(buffer);
	}
	...
	void Dispatch(uint32_t, uint32_t, uint32_t) override {
		if(m_recording && m_queueType != CommandQueueType::Copy){
			++m_dispatchCount;
		}
	}
...
public:
	...
	NullRHICommandQueue(NullRHIDevice* owner, CommandQueueType type)
		: m_owner(owner), m_type(type){}

	void Attach(NullRHIDevice* owner, CommandQueueType type){
		m_owner = owner;
		m_type = type;
	}
...
public:
	explicit NullRHISwapChain(SwapChainDesc desc)
...
public:
	...
	BufferHandle CreateBuffer(const BufferDesc& desc, std::span<const std::byte>) override {
		return desc.byteSize > 0 ? m_buffers.Create(desc) : BufferHandle{};
	}

	TextureHandle CreateTexture(const TextureDesc& desc, std::span<const std::byte>, uint32_t) override {
		return desc.width > 0 && desc.height > 0
			? m_textures.Create(desc)
			: TextureHandle{};
	}

	ShaderHandle CreateShader(const ShaderDesc& desc, std::span<const std::byte> byteCode) override {
		return !byteCode.empty() ? m_shaders.Create(desc) : ShaderHandle{};
	}

	PipelineStateHandle CreatePipelineState(const PipelineStateDesc& desc) override {
		return m_pipelines.Create(desc);
	}
	...
	const IRHICommandQueue* GetQueue(CommandQueueType type) const override {
		return const_cast<NullRHIDevice*>(this)->GetQueue(type);
	}

	std::unique_ptr<IRHICommandList> CreateCommandList(
		const CommandListCreateDesc& desc
	) override {
		return std::make_unique<NullRHICommandList>(desc.queueType);
	}

	std::unique_ptr<IRHIFence> CreateFence(uint64_t initialValue) override {
		auto state = std::make_shared<NullFenceState>();
		state->completedValue = initialValue;
		const FenceHandle handle = m_fences.Create(state);
		return std::make_unique<NullRHIFence>(handle, std::move(state));
	}
	...
	std::shared_ptr<NullFenceState> FindFence(FenceHandle handle) const {
...
inline bool NullRHICommandQueue::Submit(const QueueSubmitDesc& desc){
	...
	for(IRHICommandList* commandList : desc.commandLists){
		...
		auto* nullList = dynamic_cast<NullRHICommandList*>(commandList);
		if(!nullList || !nullList->IsClosed()) return false;
	}
...
public:
	...
	std::vector<AdapterInfo> EnumerateAdapters() const override {
		return {AdapterInfo{0, "Null Adapter", 0, 0, true}};
	}

	std::unique_ptr<IRHIDevice> CreateDevice(
		const DeviceCreateDesc& desc
	) override {
		return std::make_unique<NullRHIDevice>(desc);
	}
};

inline std::unique_ptr<IRHIBackend> CreateNullRHIBackend(){
	return std::make_unique<NullRHIBackend>();
}

inline bool RegisterNullRHIBackend(){
	return BackendRegistry::Instance().Register(
		BackendType::Null,
		&CreateNullRHIBackend
	);
}

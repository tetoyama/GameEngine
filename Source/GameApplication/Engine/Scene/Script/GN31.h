...
private:
	...
	void OnUpdate(float dt) override {
		...
		switch (m_NetWorkState) {
			...
			case NETWORKSTATE_START:
				m_ref.GetScene()->pManager->pDebug->LOG_DEBUG("GN31 Started");

				m_ref.GetScene()->pManager->pDebug->LOG_DEBUG(("ポート番号 : " + std::to_string(port)));

				if (WSAStartup(MAKEWORD(2, 2), &wsaData) != 0) {
					m_ref.GetScene()->pManager->pDebug->LOG_ERROR("WSAStartup failed");
					return;
				}
				// 初期化情報を表示（学習用）
				sprintf_s(debugString, "wVersion = %d.%d", LOBYTE(wsaData.wVersion), HIBYTE(wsaData.wVersion));
				m_ref.GetScene()->pManager->pDebug->LOG_DEBUG(debugString);
				...
				m_ref.GetScene()->pManager->pDebug->LOG_DEBUG(debugString);
				m_ref.GetScene()->pManager->pDebug->LOG_DEBUG("WSAStartup成功です");
				...
				if (listenSocket == INVALID_SOCKET
					...
					m_ref.GetScene()->pManager->pDebug->LOG_DEBUG("ソケットオープンエラー\n");
					WSACleanup();
					exit(-5);
				}
				...
				if (bind(listenSocket, (SOCKADDR*)&serverAddr, sizeof(serverAddr)) == SOCKET_ERROR) {
					m_ref.GetScene()->pManager->pDebug->LOG_DEBUG("バインドエラー\n");
				...
				if (listen(listenSocket, SOMAXCONN) == SOCKET_ERROR) {
					m_ref.GetScene()->pManager->pDebug->LOG_DEBUG("待機状態変更失敗\n");
					closesocket(listenSocket);
					WSACleanup();
					m_NetWorkState = NETWORKSTATE_CLOSE;
					return;
				...
				if (clientSocket == INVALID_SOCKET
					...
					m_ref.GetScene()->pManager->pDebug->LOG_DEBUG("accept error\n");
					closesocket(listenSocket);
					WSACleanup();
					m_NetWorkState = NETWORKSTATE_CLOSE;
					return;
				...
				m_ref.GetScene()->pManager->pDebug->LOG_DEBUG(debugString);
				m_ref.GetScene()->pManager->pDebug->LOG_DEBUG("受信を開始します\n終了は end です\n");
			...
			case NETWORKSTATE_RECV:
				{
					...
					if (recvSize == SOCKET_ERROR) {
						//m_ref.GetScene()->pManager->pDebug->LOG_DEBUG("受信エラーです\n");
						return;
					}
					recvBuffer[recvSize] = '\0'; // 文字列終端を追加
					if (strcmp(recvBuffer, "end") == 0) {
						m_ref.GetScene()->pManager->pDebug->LOG_DEBUG("クライアントが接続を切りました\n");
						m_NetWorkState = NETWORKSTATE_CLOSE;
						return;
					}
	...
	void OnStop() override {
		if (m_NetWorkState == NETWORKSTATE_RECV) {
			...
			m_ref.GetScene()->pManager->pDebug->LOG_DEBUG("GN31 Stopped");
		}
	}
};

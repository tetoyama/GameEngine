...
public:
	void Reserve(std::size_t groupCount, std::size_t instanceCount){
		...
		m_sourceGroupIndices.reserve(groupCount);
	...
	bool Build(
		...
		for(std::size_t sourceGroupIndex = 0;
			sourceGroupIndex < groups.size();
			++sourceGroupIndex){
			AppendVisibleGroup(
				sourceGroupIndex,
	...
	std::span<const std::size_t> SourceGroupIndices() const noexcept {
		return m_sourceGroupIndices;
	}
...
private:
	bool HasCapacity(
		...
		return groupCount <= m_groups.capacity() &&
			groupCount <= m_sourceGroupIndices.capacity() &&
	...
	void ReserveForBuild(
		...
		const std::size_t sourceGroupCapacityBefore =
			m_sourceGroupIndices.capacity();
		...
		if(m_groups.capacity() > groupCapacityBefore ||
			m_sourceGroupIndices.capacity() > sourceGroupCapacityBefore ||
	...
	void AppendVisibleGroup(
		...
		m_sourceGroupIndices.push_back(sourceGroupIndex);
	}
	...
	std::vector<std::size_t> m_sourceGroupIndices;

...
inline bool Button(const char* label, const ImVec2& requestedSize = ImVec2(0.0f, 0.0f), ButtonKind kind = ButtonKind::Secondary) {
	...
	if(size.x <= 0.0f) size.x = textSize.x + theme.horizontalPad * 2.0f;
...
inline bool Toggle(const char* label, bool* value) {
	...
	drawList->AddText(textPosition, ImGui::GetColorU32(theme.textPrimary), label);
	return pressed;
}
...
inline bool SectionHeader(const char* label, bool* open, float width = -1.0f) {
	...
	const float arrowX = min.x + 11.0f;
	const float arrowSize = 4.0f;
	const float angle = openAmount * 1.57079632679f;
	...
	for(ImVec2& point : points){
		...
		point = ImVec2(arrowX + x, centerY + y);
	}
	drawList->AddTriangleFilled(points[0], points[1], points[2], ImGui::GetColorU32(theme.textSecondary));

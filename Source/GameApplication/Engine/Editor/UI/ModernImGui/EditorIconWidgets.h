...
inline float PanelShortcutWidth(
...
inline bool PanelShortcutButton(
	...
	DrawEditorIcon(
		...
		iconSize,
		hovered || selected ? 1.0f : 0.72f
	);

	const ImVec4 textColor = hovered || selected
		? theme.textPrimary
		: theme.textSecondary;
...
inline bool IconButton(
	...
	if(size.x == 0.0f){
		size.x = theme.horizontalPad * 2.0f + iconSize + 7.0f + textSize.x;
	}
...
inline bool IconSectionHeader(
	...
	drawList->PushClipRect(
		...
		ImVec2(boundsMax.x - 6.0f, boundsMax.y),
		true
	);
	drawList->AddText(

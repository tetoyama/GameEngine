...
inline void DrawEditorVectorGlyph(
	...
	if(!drawList || icon == EditorIcon::Count || size <= 0.0f) return;
	...
	const float stroke = (std::max)(1.15f, size * 0.10f);
	const float thinStroke = (std::max)(1.0f, stroke * 0.80f);
	...
	auto node = [&](float x, float y, float radius = 0.078f){
		drawList->AddCircleFilled(point(x, y), size * radius, color, 12);
	};
	auto roundedRect = [&](float x0, float y0, float x1, float y1, float scale = 1.0f){
		drawList->AddRect(
			point(x0, y0),
			point(x1, y1),
			color,
	...
	case EditorIcon::Assets:
		roundedRect(0.10f, 0.34f, 0.90f, 0.82f);
	...
	case EditorIcon::Inspector:
		...
		node(0.35f, 0.27f, 0.070f);
		node(0.67f, 0.50f, 0.070f);
		node(0.45f, 0.73f, 0.070f);
		break;
	...
	case EditorIcon::Transform:
		...
		drawList->AddCircleFilled(point(0.50f, 0.50f), size * 0.060f, color, 12);
		break;
	case EditorIcon::Viewport:
		roundedRect(0.10f, 0.16f, 0.90f, 0.84f);
		line(0.23f, 0.31f, 0.40f, 0.31f, thinStroke);
		line(0.23f, 0.31f, 0.23f, 0.47f, thinStroke);
		line(0.77f, 0.69f, 0.60f, 0.69f, thinStroke);
		line(0.77f, 0.69f, 0.77f, 0.53f, thinStroke);
		break;
	case EditorIcon::Play:
		roundedRect(0.10f, 0.16f, 0.90f, 0.84f);
		drawList->AddTriangleFilled(point(0.42f, 0.32f), point(0.42f, 0.68f), point(0.70f, 0.50f), color);
		break;
	case EditorIcon::Settings:
		line(0.15f, 0.27f, 0.85f, 0.27f, thinStroke);
		line(0.15f, 0.50f, 0.85f, 0.50f, thinStroke);
		line(0.15f, 0.73f, 0.85f, 0.73f, thinStroke);
		node(0.61f, 0.27f, 0.080f);
		node(0.36f, 0.50f, 0.080f);
		node(0.69f, 0.73f, 0.080f);
		break;
	case EditorIcon::Scene:
		drawList->AddCircle(point(0.50f, 0.50f), size * 0.31f, color, 24, stroke);
		line(0.50f, 0.12f, 0.50f, 0.30f, thinStroke);
		line(0.50f, 0.70f, 0.50f, 0.88f, thinStroke);
		line(0.12f, 0.50f, 0.30f, 0.50f, thinStroke);
		line(0.70f, 0.50f, 0.88f, 0.50f, thinStroke);
		break;
	case EditorIcon::Telemetry:
		roundedRect(0.09f, 0.17f, 0.91f, 0.83f);
		line(0.17f, 0.58f, 0.31f, 0.58f, thinStroke);
		line(0.31f, 0.58f, 0.40f, 0.34f);
		line(0.40f, 0.34f, 0.52f, 0.70f);
		line(0.52f, 0.70f, 0.62f, 0.44f);
		line(0.62f, 0.44f, 0.83f, 0.44f, thinStroke);
		break;
...
inline void DrawEditorIcon(
	...
	if(!drawList || !icon.IsValid() || size <= 0.0f) return;
	if(icon.IsVector()){
		DrawEditorVectorGlyph(drawList, icon.vectorIcon, topLeft, size, alpha);
		return;
	}
	drawList->AddImage(
		icon.texture,
...
inline void DrawEditorIcon(
	...
	DrawEditorIcon(ImGui::GetWindowDrawList(), icon, topLeft, size, alpha);
}
...
inline bool PanelShortcutButton(
	...
	DrawEditorIcon(drawList, icon, ImVec2(iconX, centerY - iconSize * 0.5f), iconSize, hovered || selected ? 1.0f : 0.72f);
...
inline bool IconSectionHeader(
	...
	boundsMax.y -= inset;
	ImDrawList* drawList = ImGui::GetWindowDrawList();
	drawList->AddRectFilled(ImVec2(boundsMin.x, boundsMin.y + 1.0f), ImVec2(boundsMax.x, boundsMax.y + 1.0f), ImGui::GetColorU32(ImVec4(0, 0, 0, 0.14f)), theme.cornerRadius);
	...
	if(focused) DrawFocusRing(drawList, boundsMin, boundsMax, theme.cornerRadius);
	const float centerY = (boundsMin.y + boundsMax.y) * 0.5f;
	DrawDisclosureArrow(drawList, ImVec2(boundsMin.x + 12.0f, centerY), openAmount, ImGui::GetColorU32(hovered ? theme.textPrimary : theme.textSecondary));
	const float iconSize = 14.0f;
	const float iconX = boundsMin.x + 25.0f;
	DrawEditorIcon(drawList, icon, ImVec2(iconX, centerY - iconSize * 0.5f), iconSize, hovered ? 1.0f : 0.82f);
	...
	drawList->AddText(ImVec2(textX, centerY - textSize.y * 0.5f), ImGui::GetColorU32(theme.textPrimary), visibleLabel);
...
inline TreeRowResult IconTreeRow(
	...
	if(fill.w > 0.001f) drawList->AddRectFilled(boundsMin, boundsMax, ImGui::GetColorU32(fill), theme.cornerRadius);
	if(selectedAmount > 0.001f){
		drawList->AddRectFilled(ImVec2(boundsMin.x + 1.5f, boundsMin.y + 4.0f), ImVec2(boundsMin.x + 4.0f, boundsMax.y - 4.0f), ImGui::GetColorU32(WithAlpha(theme.accentHover, 0.95f * selectedAmount)), 1.25f);
	}
	if(focused) DrawFocusRing(drawList, boundsMin, boundsMax, theme.cornerRadius);
	const float centerY = (boundsMin.y + boundsMax.y) * 0.5f;
	if(hasChildren){
		DrawDisclosureArrow(drawList, ImVec2(boundsMin.x + 13.0f, centerY), openAmount, ImGui::GetColorU32(arrowHovered ? theme.textPrimary : theme.textSecondary));
	}
	const float iconSize = 14.0f;
	const float iconX = hasChildren ? boundsMin.x + 27.0f : boundsMin.x + 8.0f;
	DrawEditorIcon(drawList, icon, ImVec2(iconX, centerY - iconSize * 0.5f), iconSize, selected || hovered ? 1.0f : 0.76f);
	const float textX = iconX + iconSize + 6.0f;
	...
	if(badge && badge[0]){
		...
		drawList->AddRect(badgeMin, badgeMax, ImGui::GetColorU32(WithAlpha(theme.accentHover, 0.42f)), 9.0f, 0, 1.0f);
		drawList->AddText(ImVec2(badgeMin.x + 6.0f, centerY - badgeTextSize.y * 0.5f), ImGui::GetColorU32(theme.accentHover), badge);
...
inline EditorIcon ResolveDockTabIcon(const ImGuiWindow* window){
...
inline void DrawDockTabIcons(const EditorIconLibrary& icons){
	...
	for(ImGuiWindow* window : context->Windows){
		...
		for(ImGuiTabItem& tab : tabBar->Tabs){
			...
			const float tabMinX = tabBar->BarRect.Min.x + tab.Offset - tabBar->ScrollingAnim;
			const float tabMaxX = tabMinX + tab.Width;
			if(tabMaxX <= tabBar->BarRect.Min.x || tabMinX >= tabBar->BarRect.Max.x){
				break;
			}

			const bool selected = tab.ID == tabBar->SelectedTabId;
			const ImVec2 tabMin(tabMinX, tabBar->BarRect.Min.y);
			const ImVec2 tabMax(tabMaxX, tabBar->BarRect.Max.y);
			const bool hovered = ImGui::IsMouseHoveringRect(tabMin, tabMax, false);
			const float fontSize = window->FontRefSize > 0.0f
				? window->FontRefSize
				: context->FontSize;
			const float iconSize = (std::max)(9.0f, (std::min)(12.0f, fontSize * 0.80f));
			const bool hasCloseButton = (tab.Flags & ImGuiTabItemFlags_NoCloseButton) == 0;
			const float closeReserve = hasCloseButton
				? fontSize + tabBar->FramePadding.x * 2.0f + 3.0f
				: tabBar->FramePadding.x + 3.0f;
			const float contentMinX = tabMin.x + tabBar->FramePadding.x;
			const float contentMaxX = tabMax.x - closeReserve;
			if(contentMaxX - contentMinX < iconSize + 20.0f) break;

			const ImVec4 tabColor = hovered
				? ImGui::GetStyleColorVec4(ImGuiCol_TabHovered)
				: ImGui::GetStyleColorVec4(selected ? ImGuiCol_TabActive : ImGuiCol_Tab);
			ImDrawList* drawList = tabBar->Window->DrawList;
			const ImVec2 contentMin(contentMinX, tabMin.y + 2.0f);
			const ImVec2 contentMax(contentMaxX, tabMax.y - 1.0f);
			drawList->AddRectFilled(contentMin, contentMax, ImGui::GetColorU32(tabColor), 2.0f);

			const float centerY = (tabMin.y + tabMax.y) * 0.5f;
			...
			DrawEditorIcon(
				...
				iconSize,
				selected ? 0.96f : (hovered ? 0.82f : 0.58f)
			);
			...
			drawList->PushClipRect(
				ImVec2(textX, tabMin.y),
				ImVec2(contentMaxX, tabMax.y),
				true
			);
			drawList->AddText(
				ImVec2(textX, centerY - textSize.y * 0.5f),
				ImGui::GetColorU32(GetTheme().textPrimary),
				label,
				labelEnd
			);
			drawList->PopClipRect();

			const float desiredWidth = tab.ContentWidth + iconSize + 7.0f;
			if(tab.RequestedWidth < desiredWidth) tab.RequestedWidth = desiredWidth;
			break;
		}
	}
}

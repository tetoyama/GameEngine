...
int main() {
	...
	std::fprintf(stderr, "[probe] revision=%d  llmCalls=%zu\n",
		prompts::CurrentRequestRevision(), llm.GetCalls().size());
	std::fprintf(stderr, "[probe] resolvedRequest=%s\n", prompts::CurrentResolvedRequest().c_str());
	std::fprintf(stderr, "[probe] intake=%s\n",
		prompts::CurrentConversationRequestContext().at("intake").dump().substr(0, 400).c_str());
	std::fprintf(stderr, "[probe] llmScores=%s\n", result.stopInfo.dump().c_str());
	for (const auto& call : llm.GetCalls()) {
		std::size_t pos = call.first.find("役割: ");
		std::fprintf(stderr, "[probe] call: %s\n", pos == std::string::npos
			? "?" : call.first.substr(pos, 40).c_str());
	}

...
`RenderSystem.Frame.Submit`からECS走査と描画準備を分離し、WorkerがRender Packetを生成し、MainThreadが完成済みPacketを提出する構造へ移行する。
...
- [ ] API非依存`RenderPacket`定義
- [ ] Frame-local Packet Buffer
- [ ] Model / Mesh / Sprite / Particle / Terrain / Wave Packet Builder
- [ ] Packet Sort Key仕様
- [ ] Worker-local Packet Bufferと決定的Merge

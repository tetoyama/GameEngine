...
3. `RaycastWithMask` takes an exclusion mask. Player layer 1 and gameplay-trigger layer 3 are excluded from character probes. Camera probes additionally enforce enemy/boss layer 4 exclusion at runtime, producing mask `26` even when an older Scene stores `10`.
...
5. Ground snap is a velocity. The former `3.2` downward velocity generated enough contact impulse for floor friction to cancel ground acceleration. It is now a small `0.12` support bias.
6. Camera obstruction must not respond to one-frame ray hits. Occlusion now has entry/exit hysteresis, smooth shortening/return, a probe start offset, and a 2.5 minimum distance.
7. Structural mutation from Update, FixedUpdate, collision, or trigger callbacks must be queued.
8. Runtime-created game entities use queued Prefab instantiation; the dedicated Scene stores only the core game, player, camera, light, and sky entities.
9. Runtime Prefab scripts initialize safely: FixedUpdate may initialize their object state first, then the next frame performs `Start()` and `OnStart()` once.
...
- `PlatformerCameraController`: constrained follow camera, zone framing, collision shortening with hysteresis, boss framing, and clear framing.
...
- `PlatformerHud`: SpriteRenderer-based coin count, health, boss health, clear result, and restart prompt without in-game ImGui.
...
6. Boss defeat presentation, clear result, and `R`/Enter restart.
...
- Course-section materials, directional light/shadow, sky environment, Bloom post effect, particles, BGM, generated action effects, boss material telegraph, boss defeat collapse, Sprite HUD, boss bar, clear jingle, and result screen.
...
- [ ] Latest Windows Debug x64 compile check green.
...
- [x] Phase 3: coin, Sprite HUD, particles, audio, enemy stomp, damage, knockback, invulnerability, and checkpoints.
...
- [ ] Phase 7: latest Windows compile result, runtime playthrough, tuning, and final defect stabilization.

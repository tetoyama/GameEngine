...
- Static Source GPU Instance Buffer
- Player / Editor View単位Static GPU Instance Buffer
- Shadow Light View単位Static GPU Instance Buffer
...
- Static GPU Instance Reallocation / Failure / Growth Denied
...
- Static Source GPU Instance Current / Capacity / Upload / Reallocation / Failure
- 全計測値の一括Reset
...
### Static Batch GPU Instance Buffer

Active Sceneの`staticBatchReserve`を合算し、Source GPU Instance BufferへUpload前に適用する。Player / Editor / Shadow Light View単位GPU Bufferにも、そのViewのStatic Groupが参照するScene設定を合算して適用する。

`allowRuntimeGrowth`はいずれかの対象Sceneが`false`なら全体を`false`とする。必要Instance数が確保済みCapacityを超える場合はBufferを差し替えず、Upload状態を無効化する。

```text
GPU Instance Capacity不足 + Growth禁止
    -> growthDeniedCount / failedUploadCountを加算
    -> 既存GPU BufferのCapacityは維持
    -> Static Upload / Static Drawを中止
    -> Replacement SetへPacketを登録しない
    -> 通常Render Packet提出を継続
```

Static Batchは描画最適化なので、GPU Capacity不足を理由に通常Packetを捨てない。
...
- `Scene::Save`と`Scene::TempSave`はEntity列と独立した`SceneSettings.Storage`へ設定を書き込む。
- Scene初期化時はRegistry生成前にファイルから設定だけを先読みする。
- 再読込経路でもEntity生成前に設定をDecodeし、Storage Reserveを再適用する。
- 設定Nodeが存在しない旧SceneはDefault値へ戻す。
- YAML解析失敗やファイルOpen失敗では呼び出し元の設定を破壊しない。

自動回帰では、設定とEntity列を同じRootへEncodeし、`YAML::Emitter`で文字列化、一時ファイルへ保存、`SceneStoragePreloader`で先読み、`YAML::LoadFile`で再解析する一連の往復を検証する。
...
- [x] Static Source GPU Instance Reserve / Growth Policy
- [x] Player / Editor / Shadow View単位GPU Instance Reserve / Growth Policy
...
- [x] Static Batch GPU容量不足時の通常Packet Fallback
- [x] Render Packet Growth禁止時の完全Frame Safety Growth Fallback
- [x] Render Packet Safety Override Telemetry / Editor警告
- [x] YAML Node Encode / Decode回帰
- [x] YAML Emitter / File / Preloader往復回帰
- [x] 設定Nodeなし旧SceneのDefault互換
- [x] 専用Smoke Test / Workflow

## 未完了

- [ ] Editor操作によるScene Save / Load実機回帰

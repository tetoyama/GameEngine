...
`StaticBatchGpuInstanceBufferTelemetry`は`growthDeniedCount`を保持する。Editor表示への追加はStep 18 Runtime Telemetry側で継続する。
...
Active Sceneの`staticBatchReserve`を合算し、Source GPU Instance BufferへUpload前に適用する。Player / Editor View単位GPU Bufferにも、そのViewのStatic Groupが参照するScene設定を合算して適用する。
...
- [x] Player / Editor View単位GPU Instance Reserve / Growth Policy

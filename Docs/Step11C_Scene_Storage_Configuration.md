...
- Sceneごとの`renderPacketReserve`を最終`RenderPacketFrameBuffer`へ一度だけ適用する
- Sceneごとの`visibleEntityReserve`をView単位のVisibility Set構築時に適用する
- `SceneStorageSettingsPanel`からActive Sceneを選択し、Storage設定を編集・保存できる
- Scene SettingsのStorageタブでConfigured / Current / Peakを表示する
- 計測Peakをそのまま、またはSafety Margin付きでScene設定へ反映できる
- Entity RegistryのGrowth Event数を表示し、PeakとGrowth計測をリセットできる
- Component Storage、Render Packet、Visibility、Static Batchを横断した統一Growth Event集計は未完了

...
4. Pose Revision、Pose Source Model Revision、Ready Flagを更新
...
3. GPU Skinning対象はBone Constant Bufferを更新
4. Compute ShaderをDispatch
5. Output BufferをEntity固有Dynamic Vertex BufferへCopy
6. CPU Skinning対象はWorkerが生成したStaging VertexをMap / memcpy / Unmap

Revisionが一致しない場合、旧Model由来のBone Pose / CPU Stagingを破棄し、新ModelのBufferへUploadしない。

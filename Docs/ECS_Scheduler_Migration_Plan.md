...
4. [x] Step 17-C Animation CPU Build / GPU Upload分離 【実装済・実機動作確認(2026-07-11)】`RenderSystemAnimationTaskRegistrar::Register`でPose計算(`RenderSystem.Animation.Pose.Calculate` / Editor / Earliest / AnyWorker / Write:ModelRendererComponent, Read:SceneManager+ModelData)とSkinning Upload(`RenderSystem.Animation.Upload` / Editor / Early / MainThread / Write:ModelRendererComponent+ResourceService+ModelData+GraphicsContext, Read:SceneManager)を分離登録。`renderSystem.cpp:529`で`RegisterTasks`から直接登録し旧`MigrateRegisteredTasks`互換Hookは撤去済。スキニングCS生成成功・アニメモデル(Y Bot.fbx)実行を実機ログで確認(2026-07-11)。残: `AnimationUploadFailurePolicy::ShouldRecreateBuffers`は定義のみでUpload失敗時のBuffer再生成に未接続(ログ記録のみ)。Inspector経由のClip Import Main Thread専用Command化、および`Step17C_...md`の実機検証チェックリスト(GPU/CPU見た目一致・複数Entity同時再生)は未消化。
5. [~] Step 17-D Terrain CPU Mesh Build / GPU Upload分離 【実装済・要ビルド確認(2026-07-11)】17-Cパターンを踏襲し、`TerrainSystem.Mesh.Build`(Render/Earliest/AnyWorker/純CPU)と`TerrainSystem.Mesh.Upload`(Render/Early/MainThread/GPU)へ分離。新規`TerrainMeshBuilder.h`/`TerrainMeshUpload.h`/`TerrainTaskRegistrar.h`、`terrainComponent.h`にstaging+signature追加、旧同期`CreateMesh`撤去。signature再照合で古いstaging破棄。サブエージェント2段(実装+コンパイル観点レビュー)でBlockerなし確認済だが、**環境でビルド不可のためVS実機ビルド確認が残**。起動時同期メッシュ生成を撤去し初回は最初のRenderフレームで生成される挙動変更あり。

...
path = "Source/GameApplication/Service/Graphics/RHI/RHIBackend.h"
text = load(path)
text = replace_once(
    text,
...
""",
    "DeviceCreateDesc software adapter opt-in",
)
save(path, text)


path = "Source/GameApplication/Service/Graphics/RHI/D3D11/D3D11RHIBackend.h"
text = load(path)
text = replace_once(
    text,
...
std::span<const std::byte> BlobBytes(const ID3DBlob& blob){
	return {
...
'''
save("Tests/D3D11RHIRealTriangleTest.cpp", test)


workflow = '''name: D3D11 RHI Real Triangle Smoke

on:
  pull_request:
    branches: [master]
    paths:
      - "Tests/D3D11RHIRealTriangleTest.cpp"
      - "Source/GameApplication/Service/Graphics/RHI/**"
      - ".github/workflows/d3d11-rhi-real-triangle.yml"
  workflow_dispatch:
  ...
  compile:
    ...
    steps:
      ...
      - uses: ilammy/msvc-dev-cmd@v1
        with:
          arch: x64
      - name: Compile real triangle smoke
        shell: cmd
        run: cl /nologo /std:c++20 /EHsc /W4 /MTd /I Source/GameApplication Tests/D3D11RHIRealTriangleTest.cpp /Fe:d3d11-rhi-real-triangle.exe
  ...
  run:
    ...
    steps:
      ...
      - uses: ilammy/msvc-dev-cmd@v1
        with:
          arch: x64
      - name: Compile and run real triangle smoke
        shell: cmd
        run: |
          cl /nologo /std:c++20 /EHsc /W4 /MTd /I Source/GameApplication Tests/D3D11RHIRealTriangleTest.cpp /Fe:d3d11-rhi-real-triangle.exe
          d3d11-rhi-real-triangle.exe
'''
save(".github/workflows/d3d11-rhi-real-triangle.yml", workflow)
...
text = replace_once(
    ...
    "D3D11 real triangle completion link",
)
text = replace_once(
    text,
...
""",
    "Current work position after D3D11 triangle",
)
save(path, text)


completion = """# Step 16-G: D3D11 Real Triangle Completion
...
"""
save("Docs/Step16G_D3D11_Real_Triangle_Completion.md", completion)

print("D3D11 real triangle migration applied")
